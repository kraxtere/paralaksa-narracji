---
id: 2026-09-22-trump-onz-iran
tytul: Przemówienie Trumpa na 81. sesji Zgromadzenia Ogólnego ONZ (Iran, groźba „unicestwienia”)
status: kandydat
powod_odrzucenia: null
droga: od_wiadomosci        # z naszej bazy: CGTN, tagesschau, Spiegel, Guardian, Al Jazeera, Onet
dziedzina: [polityka_miedzynarodowa, bliski_wschod]
forma: dwie_opowiesci       # para z tego samego gatunku: komentarz vs komentarz/analiza

fakt:
  czas: null                # przemówienie 22.09 (Nowy Jork), godzina do ustalenia
  opis: >-
    22 września 2026 Donald Trump powiedział w ONZ, że stoi przed „wielką decyzją”: porozumienie z Iranem kończące wojnę
    albo „unicestwienie” Republiki Islamskiej. Porozumienia spodziewa się po listopadowych wyborach do Kongresu.
    Tego samego dnia delegacje USA i Iranu rozmawiały w Nowym Jorku.
  zrodlo_pierwotne: null     # zapis przemówienia (whitehouse.gov / un.org) do dopięcia

stan_wiedzy_zmienial_sie: false
os_czasu: []

relacje:
  - id: r1
    kraj: DE
    kto: tagesschau (komentarz Martina Ganslmeiera)
    rola: redakcja
    gatunek: komentarz
    link: https://www.tagesschau.de/kommentar/trump-rede-un-kommentar-100.html
    publikacja: '2026-09-22T22:44+00:00'   # z RSS
    aktualizacja: null
    porownywana_wersja: 'tytuł i lead z RSS pobrane 2026-09-23'
    archiwum: {link: 'https://web.archive.org/web/20260923095832/https://www.tagesschau.de/kommentar/trump-rede-un-kommentar-100.html', wykonano: '2026-09-23T09:58Z'}
    naglowek: 'Trump bei UN-Vollversammlung: Mehr Staatsmann und weniger Konfrontation'
    tlumaczenie: 'Trump na Zgromadzeniu Ogólnym ONZ: bardziej mąż stanu, mniej konfrontacji'
    zostawia_z: 'Trump „nietypowo zdyscyplinowany”, z powodu presji krajowej.'
    sprawdzil: null          # z RSS
  - id: r2
    kraj: UK
    kto: The Guardian
    rola: redakcja
    gatunek: analiza
    link: https://www.theguardian.com/us-news/2026/sep/22/donald-trump-unga-speech
    publikacja: '2026-09-22T18:23+00:00'   # z RSS
    aktualizacja: null
    porownywana_wersja: 'tytuł i lead z RSS pobrane 2026-09-23'
    archiwum: {link: 'https://web.archive.org/web/20260922185635/https://www.theguardian.com/us-news/2026/sep/22/donald-trump-unga-speech', wykonano: '2026-09-22T18:56Z'}
    naglowek: 'Trump’s threat to annihilate Iran to UN general assembly is stunning, even for him'
    tlumaczenie: 'Groźba Trumpa, że unicestwi Iran, rzucona na forum ONZ jest szokująca, nawet jak na niego'
    zostawia_z: 'Groźba bez precedensu (porównanie z Chruszczowem).'
    sprawdzil: null          # z RSS
  - id: r3
    kraj: DE
    kto: Spiegel
    rola: redakcja
    gatunek: analiza
    link: https://www.spiegel.de/ausland/donald-trump-bei-der-uno-ein-bisschen-drohen-ein-bisschen-kuscheln-a-9dafdd6f-d8ac-489e-986b-94242834a103
    publikacja: '2026-09-23T00:20+00:00'   # z RSS
    aktualizacja: null
    porownywana_wersja: 'tytuł i lead z RSS pobrane 2026-09-23'
    archiwum: {link: 'https://web.archive.org/web/20260923010927/https://www.spiegel.de/ausland/donald-trump-bei-der-uno-ein-bisschen-drohen-ein-bisschen-kuscheln-a-9dafdd6f-d8ac-489e-986b-94242834a103', wykonano: '2026-09-23T01:09Z'}
    naglowek: 'Donald Trump bei der Uno: Ein bisschen drohen, ein bisschen kuscheln'
    tlumaczenie: 'Donald Trump w ONZ: trochę grozić, trochę się przytulać'
    zostawia_z: '„Dziwnie oswojona wersja” Trumpa; porażek tym nie przykryje.'
    sprawdzil: null          # z RSS
  - id: r4
    kraj: CN
    kto: CGTN
    rola: redakcja
    gatunek: wiadomosc
    link: https://newsaf.cgtn.com/news/2026-09-23/Trump-warns-in-UN-speech-he-could-annihilate-Iran-without-peace-deal-1QEGX6APjmU/p.html
    publikacja: '2026-09-22T16:56+00:00'   # z RSS
    aktualizacja: null
    porownywana_wersja: 'tytuł i lead z RSS pobrane 2026-09-23'
    archiwum: {link: null, wykonano: null}
    naglowek: "Trump warns in UN speech he could 'annihilate' Iran without peace deal"
    tlumaczenie: 'Trump ostrzega w ONZ, że może „unicestwić” Iran bez porozumienia pokojowego'
    zostawia_z: 'Rzeczowy zapis groźby z warunkiem.'
    sprawdzil: null          # z RSS; CGTN to nadawca państwowy ChRL (nie jest objęty sankcjami UE)
  - id: r5
    kraj: US
    kto: NBC News
    rola: redakcja
    gatunek: wiadomosc
    link: https://www.nbcnews.com/politics/trump-administration/trump-address-united-nations-general-assembly-iran-war-rcna599085
    publikacja: '2026-09-22T09:00+00:00'   # datePublished: zapowiedź przed przemówieniem pod tym samym adresem
    aktualizacja: '2026-09-23T07:15+00:00'
    porownywana_wersja: 'kopia 2026-09-23 21:11 UTC. Kopia 2026-09-22 11:51 UTC ma zapowiedź: Trump to address the U.N. General Assembly as Iran war rumbles on'
    archiwum: {link: 'https://web.archive.org/web/20260923211149/https://www.nbcnews.com/politics/trump-administration/trump-address-united-nations-general-assembly-iran-war-rcna599085', wykonano: '2026-09-23T21:11Z'}
    naglowek: 'Trump tells U.N. he could ‘annihilate’ Iran but expects a deal after the midterm elections'
    tlumaczenie: 'Trump mówi ONZ, że może „unicestwić” Iran, ale spodziewa się porozumienia po wyborach'
    zostawia_z: 'Groźba i porozumienie w jednym zdaniu.'
    sprawdzil: null          # tytuł z wyników wyszukiwania
  - id: r6
    kraj: PL
    kto: Onet (opinia, tekst przedrukowany)
    rola: redakcja
    gatunek: komentarz
    link: https://wiadomosci.onet.pl/swiat/iran-moze-zlozyc-trumpowi-oferte-ktorej-nie-bedzie-mogl-odrzucic-opinia/33pv5b0
    publikacja: '2026-09-23T06:34+00:00'   # z RSS
    aktualizacja: null
    porownywana_wersja: 'tytuł z RSS; adres URL sugeruje inny, wcześniejszy tytuł („Iran może złożyć Trumpowi ofertę…”)'
    archiwum: {link: 'https://web.archive.org/web/20260923091021/https://wiadomosci.onet.pl/swiat/iran-moze-zlozyc-trumpowi-oferte-ktorej-nie-bedzie-mogl-odrzucic-opinia/33pv5b0', wykonano: '2026-09-23T09:10Z'}
    naglowek: 'Donald Trump robi z siebie pośmiewisko w sprawie Iranu [OPINIA]'
    tlumaczenie: null
    zostawia_z: 'Groźba nie budzi strachu, tylko drwinę.'
    sprawdzil: null          # z RSS

kontrasty:
  - miedzy: [r1, r2]
    rodzaj: dobor_slow
    opis: 'To samo przemówienie: „bardziej mąż stanu, mniej konfrontacji” (tagesschau) i groźba „szokująca nawet jak na niego” (Guardian).'
    zastrzezenia: 'Oba teksty to komentarz albo analiza, a nie relacja. tagesschau pisał ok. 4,5 h później i mógł już znać rozmowy USA–Iran. Obaj autorzy mogą mieć rację jednocześnie (spokojny ton, jedna skrajna groźba).'
  - miedzy: [r2, r6]
    rodzaj: dobor_slow
    opis: 'Ta sama groźba jako szok (Guardian) albo jako powód do drwiny (Onet).'
    zastrzezenia: 'Onet: opinia, możliwy przedruk; nagłówek zmieniony względem adresu URL.'

jak_szukano: ['nasza baza (RSS 22–23.09): tagesschau, Guardian, Spiegel, CGTN, Al Jazeera, BBC, Onet', 'wyszukiwarka WWW en: ABC, NBC, CBS, CNN, Axios, Fox']
---

## Fakt
Przemówienie w ONZ 22.09: alternatywa „porozumienie albo unicestwienie”, porozumienie przewidywane po wyborach do Kongresu.
Tego samego dnia Trump potwierdził trzygodzinną rozmowę USA–Iran w Nowym Jorku.

## Przekaz
Jedno przemówienie wywołało dwie oceny tonu. Dla jednych to „oswojony” Trump, dla innych groźba bez precedensu.
Różnica dotyczy tego, czy ważniejszy jest ton całości, czy jedno zdanie.

## Hipoteza odbioru
Widz zobaczy, że nagłówek komentarza to wybór jednego zdania z całego przemówienia. Pokazać obie połowy tego zdania.

## Ilustracja
Mównica z dwoma mikrofonami: jeden cichy, drugi z czerwoną falą dźwięku. Bez wizerunku Trumpa i bez karykatury.

## Uczciwość odcinka
- [ ] Zapis przemówienia (dokładny cytat, godzina).
- [ ] Nie zestawiać komentarza z depeszą; para r1/r2 to komentarz i analiza.
- [ ] Człowiek otwiera komentarz tagesschau: czy odnosi się do groźby.

## Notatki z pilota
- Czas pracy: ok. 10 min (Claude).
- Co było najbardziej żmudne: pilnowanie gatunków. Najostrzejsze nagłówki są w komentarzach.
- Czego zabrakło w karcie: nic.
